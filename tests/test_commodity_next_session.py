...
class CommodityNextSessionTests(unittest.TestCase):
    ...
    def test_completed_session_builds_next_day_buy_breakout(self):
        ...
        self.assertEqual(plan["action"], "BUY CE")
        self.assertEqual(plan["underlying_direction"], "BULLISH")
    ...
    def test_bearish_session_builds_buy_pe_not_sell(self):
        monday = date(2026, 8, 24)
        rows = make_session(date(2026, 8, 21), direction=-1, start=105.0)
        rows += make_session(monday, direction=-1, start=100.0)
        plan = build_next_session_plan("CRUDEOIL", rows, monday, date(2026, 8, 25), 1.0)
        self.assertEqual(plan["status"], "SETUP")
        self.assertEqual(plan["action"], "BUY PE")
        self.assertEqual(plan["underlying_direction"], "BEARISH")
        self.assertLessEqual(plan["features"]["directional_score"], -3)
        self.assertLess(plan["entry"], plan["features"]["session_low"])
    ...
    def test_target_is_scored_only_after_entry(self):
        plan = {"status": "SETUP", "action": "BUY CE", "underlying_direction": "BULLISH", "entry": 101.0, "stop_loss": 100.0, "target1": 102.5}
    ...
    def test_no_trigger_is_not_counted_as_loss(self):
        plan = {"status": "SETUP", "action": "BUY PE", "underlying_direction": "BEARISH", "entry": 99.0, "stop_loss": 100.0, "target1": 97.5}
