...
def test_collector_uses_latest_recent_session_on_weekend():
    friday_rows=[
        ["2026-08-28T18:00:00+05:30",1150,1160,1148,1155,120],
    ]
    ce=contract("CE",1150)
    pe=contract("PE",1150)
    ...
    async def run():
        ...
        with patch(
            ...
            return await collect_copper_option_candles(
                object(),underlying,option,
                now=datetime(2026,8,29,18,0,tzinfo=IST),
            )
...
def test_collector_rejects_stale_underlying_session():
    old_rows=[
        ["2026-08-20T18:00:00+05:30",1150,1160,1148,1155,120],
    ]

    async def run():
        underlying=UnderlyingStore(old_rows)
        option=OptionStore()
        with patch(
            "app.commodity_option_candle_collector.fetch_mcx_option_master",
            new=AsyncMock(),
            ...
            result=await collect_copper_option_candles(
                object(),underlying,option,
                now=datetime(2026,8,27,18,0,tzinfo=IST),
            )
            return result,master

    result,master=asyncio.run(run())
    assert result["status"]=="STALE_UNDERLYING_SESSION"
    assert result["session_age_days"]==7
    assert result["upserted"]==0
    master.assert_not_awaited()
