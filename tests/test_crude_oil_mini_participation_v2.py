...
class CrudeOilMiniParticipationV2Tests(unittest.TestCase):
    ...
    def test_low_activity_cannot_vote_even_with_price_and_oi_alignment(self):
        result = build_participation_observation(
            _candles(),
            click_timestamp=CLICK,
            snapshot={"time_adjusted_relative_volume": 0.8, "session_vwap_gap_pct": 0.5},
            profile={"participation_confirming": 1.2},
        )
        self.assertEqual(result["state"], "LOW_OR_NORMAL_PARTICIPATION")
        self.assertFalse(result["counts_for_direction"])
