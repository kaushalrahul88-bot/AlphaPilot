...
from app.copper_commodity_brain_prospective_startup import (
    register_copper_commodity_brain_prospective_startup,
)
...
class CopperCommodityBrainProspectiveStartupTests(unittest.IsolatedAsyncioTestCase):
    async def test_empty_database_url_is_noop(self):
        app = _FakeApp()
        register_copper_commodity_brain_prospective_startup(app, _Settings(""))
        self.assertIsNotNone(app.startup)

        with patch(
            "app.copper_commodity_brain_prospective_startup.initialize_store",
