"""Durable internal API for frozen four-stock point-in-time context replay."""
...
from .fno_four_stock_context_enrichment_v1 import PROTOCOL_ID, architecture_contract
from .fno_four_stock_context_replay_v1 import run_four_stock_context_replay
from .providers.factory import get_provider

UTC = timezone.utc
RUN_TABLE = "fno_four_stock_context_replay_runs_v1"
...
def _baseline_sync(url):
    with _connect(url) as connection:
        with connection.cursor() as cursor:
            cursor.execute(
                f"SELECT result_json FROM {BASELINE_TABLE} "
                "WHERE protocol_id=%s AND status='COMPLETED' "
                "AND jsonb_array_length(COALESCE(result_json->'rows','[]'::jsonb))=1600 "
...
def _summary(run):
    ...
    if run.get("status") == "COMPLETED":
        payload.update({
            ...
            "enriched_summary": result.get("enriched_summary"),
            "comparison": result.get("comparison"),
            "context_audit": result.get("context_audit"),
            "by_stock": result.get("by_stock"),
        })
...
def register_fno_four_stock_context_replay_routes(app, settings, collector_auth):
    if getattr(app.state, "fno_four_stock_context_replay_registered", False):
        return
    app.state.fno_four_stock_context_replay_registered = True

    @app.post("/v1/internal/fno/four-stock-context-replay/start")
    ...
    @app.get("/v1/internal/fno/four-stock-context-replay/status")
    ...
    @app.get("/v1/internal/fno/four-stock-context-replay/result")
