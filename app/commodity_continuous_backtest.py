...
async def _run_contract_window(provider, symbol, contract, window_start, window_end, min_rr, strength_threshold, slippage_bps, cost_bps):
    ...
    effective_start=max(window_start,raw_start) if raw_start else None
    effective_end=min(window_end,raw_end) if raw_end else None
...
async def run_continuous_commodity_backtest(provider, symbol, days=180, min_rr=1.5, strength_threshold=65.0, slippage_bps=2.0, cost_bps=2.0):
    ...
    used=[]; skipped=[]; all_trades=[]; previous_expiry=None
    for contract in selected:
        ...
        try:
            ...
            if result["candles"].get("5m",0)==0 or result.get("effective_days",0)<=0:
            ...
            used.append({"contract":contract["trading_symbol"],"expiry":contract["expiry_date"],"window_start":result["window_start"],"window_end":result["window_end"],"effective_start":result["effective_start"],"effective_end":result["effective_end"],"effective_days":result["effective_days"],"candles":result["candles"],"trades":len(result["trades"])})
            all_trades.extend(result["trades"])
        except Exception as exc:
            skipped.append({"contract":contract["trading_symbol"],"expiry":contract["expiry_date"],"reason":str(exc)[:300]})

    all_trades.sort(key=lambda t:t["entry_time"])
    historical_days=round(sum(float(x.get("effective_days") or 0) for x in used),1)
    historical_coverage_pct=round(min(100.0,(historical_days/days*100.0) if days else 0.0),1)
    ...
    confidence="HIGH" if rollover_valid and historical_coverage_pct>=80 else "MEDIUM" if rollover_valid else "LOW"
    ...
    limitations=[
        "Historical candle span and rollover validity are reported separately; a long history for one contract is not treated as a continuous futures series.",
    ...
    return {"mode":"MCX_CONTINUOUS_AVAILABLE_CONTRACTS","classification":classification,"rollover_valid":rollover_valid,"coverage_confidence":confidence,"symbol":symbol,"requested_days":days,"requested_start":requested_start.isoformat(),"requested_end":requested_end.isoformat(),"actual_start":actual_start.isoformat() if actual_start else None,"actual_end":actual_end.isoformat() if actual_end else None,"coverage_days":historical_days,"coverage_ratio_pct":historical_coverage_pct,"historical_coverage_days":historical_days,"historical_coverage_pct":historical_coverage_pct,"contracts_discovered":len(contracts),"contracts_selected":len(selected),"contracts_used":used,"contracts_skipped":skipped,"summary":_summary(all_trades),"by_action":{"BUY":_summary([t for t in all_trades if t["action"]=="BUY"]),"SELL":_summary([t for t in all_trades if t["action"]=="SELL"])},"trades":all_trades[-300:],"limitations":limitations}
