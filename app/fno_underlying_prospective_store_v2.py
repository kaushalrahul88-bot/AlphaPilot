"""Immutable Postgres tape for prospective underlying-only F&O V2 episodes."""
...
from .fno_underlying_prospective_v2 import PROTOCOL_ID

UTC = timezone.utc
EPISODE_TABLE = "fno_underlying_prospective_episodes_v2"
OUTCOME_TABLE = "fno_underlying_prospective_outcomes_v2"
PROVENANCE_ID = "FNO_UNDERLYING_PROSPECTIVE_POSTGRES_V2"
...
CREATE TABLE IF NOT EXISTS {EPISODE_TABLE} (
    ...
    capture_latency_seconds NUMERIC NOT NULL CHECK (capture_latency_seconds >= 0),
    ...
    move_potential JSONB NOT NULL,
    move_potential_influenced_decision BOOLEAN NOT NULL CHECK (move_potential_influenced_decision = FALSE),
    ...
    CHECK (capture_started_at >= capture_slot_at),
    CHECK (captured_at >= capture_started_at)
);
CREATE INDEX IF NOT EXISTS fno_underlying_prospective_v2_slot_idx
...
CREATE INDEX IF NOT EXISTS fno_underlying_prospective_v2_outcome_due_idx
    ON {OUTCOME_TABLE} (outcome_due_at DESC);
"""

IMMUTABILITY_FUNCTION = "fno_underlying_prospective_reject_mutation_v2"
IMMUTABILITY_SQL = f"""
CREATE OR REPLACE FUNCTION {IMMUTABILITY_FUNCTION}()
RETURNS trigger AS $$
BEGIN
    RAISE EXCEPTION 'prospective underlying F&O V2 research tape is immutable';
END;
$$ LANGUAGE plpgsql;
"""
for _table, _prefix in (
    (EPISODE_TABLE, "fno_underlying_prospective_v2_episode"),
    (OUTCOME_TABLE, "fno_underlying_prospective_v2_outcome"),
):
    IMMUTABILITY_SQL += f"""
DROP TRIGGER IF EXISTS {_prefix}_reject_row_mutation ON {_table};
CREATE TRIGGER {_prefix}_reject_row_mutation
BEFORE UPDATE OR DELETE ON {_table}
FOR EACH ROW EXECUTE FUNCTION {IMMUTABILITY_FUNCTION}();
DROP TRIGGER IF EXISTS {_prefix}_reject_truncate ON {_table};
CREATE TRIGGER {_prefix}_reject_truncate
...
def _utc(value: Any) -> datetime:
    ...
    if stamp.tzinfo is None or stamp.utcoffset() is None:
        raise ValueError("prospective timestamp must be timezone-aware")
...
def _episode_params(record: Mapping[str, Any]) -> dict[str, Any]:
    if record.get("protocol_id") != PROTOCOL_ID:
        raise ValueError("unexpected underlying prospective V2 protocol")
    ...
    if record.get("future_outcome_present_in_decision") is not False:
        raise ValueError("future outcome cannot be present in decision")
    if record.get("outcome_used_for_decision") is not False:
        raise ValueError("outcome cannot influence frozen decision")
    if record.get("move_potential_influenced_decision") is not False:
        raise ValueError("move-potential diagnostic cannot influence V2 action")
    for key in (
        "option_chain_used", "option_premium_used", "option_oi_iv_greeks_used",
        "futures_used", "broker_orders_created", "live_execution",
    ):
        if record.get(key) is not False:
            raise ValueError(f"prospective underlying V2 forbids {key}")
    if float(record.get("capital_committed") or 0) != 0:
        raise ValueError("prospective underlying V2 forbids capital")

    slot = _utc(record["capture_slot_at"])
    started = _utc(record["capture_started_at"])
    captured = _utc(record["captured_at"])
    if not slot <= started <= captured:
        raise ValueError("invalid capture timestamp ordering")
    reference = float(record["reference_price"])
    if reference <= 0:
        raise ValueError("reference_price must be positive")
    move_potential = record.get("move_potential") or {}
    if move_potential.get("diagnostic_only") is not True or move_potential.get("influences_action") is not False:
        raise ValueError("move-potential payload must remain diagnostic-only")
    ...
    return {
        ...
        "move_potential": _canonical(move_potential),
...
def _outcome_params(record: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if resolved < due:
        raise ValueError("outcome cannot resolve before due time")
    return {
        "episode_id": str(record["episode_id"]),
        "horizon_code": code,
        "outcome_due_at": due,
...
class FnoUnderlyingProspectiveStoreV2:
    ...
    def _insert_episode_sync(self, record: Mapping[str, Any]) -> dict[str, Any]:
        p = _episode_params(record)
        sql = f"""
        INSERT INTO {EPISODE_TABLE} (
            episode_id,protocol_id,capture_slot_at,capture_started_at,captured_at,
            capture_latency_seconds,underlying_symbol,batch_index,reference_price,
            research_action,technical_status,multi_timeframe_score,move_potential,
            move_potential_influenced_decision,live_execution_enabled,
            broker_orders_created,options_used,futures_used,capital_committed,
            payload,payload_hash,provenance_id
        ) VALUES (
            %(episode_id)s,%(protocol_id)s,%(capture_slot_at)s,%(capture_started_at)s,%(captured_at)s,
            %(capture_latency_seconds)s,%(underlying_symbol)s,%(batch_index)s,%(reference_price)s,
            %(research_action)s,%(technical_status)s,%(multi_timeframe_score)s,%(move_potential)s::jsonb,
            %(move_potential_influenced_decision)s,%(live_execution_enabled)s,
            %(broker_orders_created)s,%(options_used)s,%(futures_used)s,%(capital_committed)s,
        ...
        with _connect(self.database_url) as conn:
            with conn.cursor() as cur:
                ...
                row = cur.fetchone()
            conn.commit()
        return {"status": "INSERTED" if row else "ALREADY_EXISTS", "episode_id": p["episode_id"]}
    ...
    def _pending_sync(self, since: datetime, limit: int) -> list[dict[str, Any]]:
        sql = f"""
        SELECT e.episode_id,e.capture_slot_at,e.captured_at,e.underlying_symbol,
               e.reference_price,e.research_action,e.payload,
        ...
        GROUP BY e.episode_id,e.capture_slot_at,e.captured_at,e.underlying_symbol,
                 e.reference_price,e.research_action,e.payload
        ...
        return [{
            "episode_id": row[0],
            "capture_slot_at": row[1].astimezone(UTC),
            "captured_at": row[2].astimezone(UTC),
    ...
    def _insert_outcome_sync(self, record: Mapping[str, Any]) -> dict[str, Any]:
        p = _outcome_params(record)
        sql = f"""
        INSERT INTO {OUTCOME_TABLE} (
            episode_id,horizon_code,outcome_due_at,resolved_at,resolution_status,
            classification,end_price,raw_return_pct,directional_return_pct,
            max_up_pct,max_down_pct,mfe_pct,mae_pct,max_abs_excursion_pct,
            payload,payload_hash,provenance_id
        ) VALUES (
            %(episode_id)s,%(horizon_code)s,%(outcome_due_at)s,%(resolved_at)s,%(resolution_status)s,
            %(classification)s,%(end_price)s,%(raw_return_pct)s,%(directional_return_pct)s,
            %(max_up_pct)s,%(max_down_pct)s,%(mfe_pct)s,%(mae_pct)s,%(max_abs_excursion_pct)s,
            %(payload)s::jsonb,%(payload_hash)s,%(provenance_id)s
        ) ON CONFLICT (episode_id,horizon_code) DO NOTHING RETURNING episode_id;
        """
        with _connect(self.database_url) as conn:
            with conn.cursor() as cur:
                cur.execute(sql, p)
                row = cur.fetchone()
            conn.commit()
        return {"status": "INSERTED" if row else "ALREADY_EXISTS", "episode_id": p["episode_id"], "horizon_code": p["horizon_code"]}
    ...
    def _status_sync(self) -> dict[str, Any]:
        ...
        return {
            ...
            "first_capture_slot_at": row[2].astimezone(UTC).isoformat() if row[2] else None,
            "last_capture_slot_at": row[3].astimezone(UTC).isoformat() if row[3] else None,
            "database_immutable": True,
...
def architecture_contract() -> dict[str, Any]:
    return {
        ...
        "outcome_table": OUTCOME_TABLE,
        "database_immutable": True,
        "move_potential_stored": True,
        "move_potential_can_influence_action": False,
        "options_used": False,
        "futures_used": False,
        "live_execution": False,
        "capital_committed": 0,
