"""Resolve V2 prospective underlying episodes from exact completed 5-minute paths."""
...
def _classification(action: str, metrics: dict[str, Any]) -> str:
    ...
    return (
        "NO_TRADE_LARGE_MOVE"
        if excursion >= NO_TRADE_LARGE_MOVE_PCT
        else "NO_TRADE_QUIET"
    )


async def resolve_due_underlying_outcomes_v2(
    provider,
    store,
    *,
    ...
    pending = await store.pending_episodes(
        now - timedelta(days=LOOKBACK_DAYS), limit=limit
    )
    ...
    for episode, code, due in due_items:
        ...
        if symbol in fetch_errors:
            ...
            continue
        exact = _exact_future_bars(
            histories.get(symbol, []), episode["capture_slot_at"], due
        )
        if exact is None:
            incomplete.append({
                "episode_id": episode["episode_id"],
        ...
        record = {
            ...
            "move_potential_used_for_resolution_only": False,
...
def architecture_contract() -> dict[str, Any]:
    return {
        "version": "FNO_UNDERLYING_PROSPECTIVE_RESOLVER_V2",
        "exact_5m_path_required": True,
        "horizons": list(HORIZON_CODES),
        "partial_paths_persisted": False,
        "move_potential_can_change_outcome": False,
        "option_data_used": False,
        "futures_data_used": False,
