...
def session_vwap(c):
    # use only candles from latest trading date
    if not c:return 0
...
def volume_ratio_same_slot(c):
    """Compare latest bar with previous bars at same clock-time, then cap extreme ratios."""
    if not c:return 1.0,1.0
    latest=c[-1]; slot=str(latest[0])[11:16]
    peers=[x[5] for x in c[:-1] if str(x[0])[11:16]==slot and x[5]>0][-20:]
    if len(peers)<3:
        peers=[x[5] for x in c[-21:-1] if x[5]>0]
...
def analyze_candles(symbol,candles,min_rr=1.5):
    ...
    # Trend max +/-25, requiring medium-term alignment for highest score.
    ...
    # Momentum max +/-20 with explicit conflict penalties.
    ...
    if 0<=dist_res<0.75:
        struct-=5; warnings.append("Entry is close to resistance")
    if 0<=dist_sup<0.75:
        struct+=3

    # Time-of-day normalized volume. No 15-point gift for anomalous raw spikes.
    ...
    if pat in ("BULLISH_ENGULFING","HAMMER"): price_action+=6
        ...
        price_action-=4
        warnings.append("Doji signals indecision")

    # Weighted evidence, no free 50-point boost. 50 is neutral after normalization.
    max_abs=25+20+20+10+8+8
    bias=trend+momentum+struct+volume+volatility+price_action
    alpha=50+(bias/max_abs)*50
    ...
    # Hard gates: scores >90 require exceptional alignment; normal setups need 3 families.
    if alpha>=68 and long_conf>=3 and momentum>-8:
        direction="LONG"
    elif alpha<=32 and short_conf>=3 and momentum<8:
        direction="SHORT"
    ...
    base={
      ...
      "family_scores":{"trend":trend,"momentum":momentum,"structure":struct,"volume":volume,
                       "volatility":volatility,"price_action":price_action},
      "ema9":round(e9,2),"ema20":round(e20,2),"ema50":round(e50,2),
      "ema200":round(e200,2) if e200 is not None else None,"vwap":round(vw,2),
      "rsi14":round(r,2),"macd":round(ml,4),"macd_signal":round(ms,4),"macd_hist":round(mh,4),
      "atr14":round(a,2),"bollinger_upper":round(bu,2),"bollinger_mid":round(bm,2),
      "bollinger_lower":round(bl,2),"volume_ratio_raw":round(rawvr,2),
      "volume_ratio_capped":round(vr,2),"market_structure":st,
      "recent_support":round(sup,2),"recent_resistance":round(res,2),
      "distance_to_resistance_atr":round(dist_res,2),"distance_to_support_atr":round(dist_sup,2),
      "candle_pattern":pat,"confirmations":{"long":long_conf,"short":short_conf},
      "reasons":reasons,"warnings":warnings,"clean_candles":len(c)
    }
    if direction=="NO_TRADE":
        return {**base,"status":"NO_TRADE","reason":"Confluence threshold not met"}

    risk=max(a*1.25,last*.003)
    if direction=="LONG":
        stop=min(last-risk,sup-.15*a) if sup<last else last-risk
        rrisk=last-stop; t1=last+rrisk*min_rr; t2=last+rrisk*max(2,min_rr+.5)
    else:
        stop=max(last+risk,res+.15*a) if res>last else last+risk
        rrisk=stop-last; t1=last-rrisk*min_rr; t2=last-rrisk*max(2,min_rr+.5)
    return {**base,"status":"SETUP","direction":direction,"entry":round(last,2),
            "stop_loss":round(stop,2),"target1":round(t1,2),"target2":round(t2,2),
            "risk_reward":round(min_rr,2),
            "note":"Alpha Score is a normalized confluence score, not probability of profit."}
