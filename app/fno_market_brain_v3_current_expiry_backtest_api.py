"""Durable internal API for evaluating the frozen V3 current-expiry dataset."""
...
from .fno_market_brain_v3_current_expiry_backtest import (
    PROTOCOL_ID,
    SOURCE_PROTOCOL_ID,
...
RUN_TABLE = "fno_market_brain_v3_current_expiry_backtest_runs"
SQL = f"""
...
CREATE INDEX IF NOT EXISTS fno_v3_current_expiry_backtest_started_idx
...
def register_fno_market_brain_v3_current_expiry_backtest_routes(app, settings, collector_auth):
    if getattr(app.state, "fno_market_brain_v3_current_expiry_backtest_registered", False):
        return
    app.state.fno_market_brain_v3_current_expiry_backtest_registered = True

    @app.post("/v1/internal/fno/v3-current-expiry-backtest/start")
    async def start(x_collector_token: str | None = Header(default=None)):
        ...
        if latest and latest.get("status") == "RUNNING":
            if latest.get("source_run_id") != source.get("run_id"):
                raise HTTPException(409, detail={"status": "OLDER_SOURCE_EVALUATION_STILL_RUNNING"})
    ...
    @app.get("/v1/internal/fno/v3-current-expiry-backtest/status")
    ...
    @app.get("/v1/internal/fno/v3-current-expiry-backtest/result")
