"""Durable internal API for frozen V6 Historical Holdout B dataset build."""
...
from .fno_market_brain_v6_holdout_b_dataset import (
    ...
    build_holdout_b_dataset,
)
from .providers.factory import get_provider

UTC = timezone.utc
RUN_TABLE = "fno_market_brain_v6_holdout_b_dataset_runs"
CACHE_TABLE = "fno_market_brain_v6_holdout_b_history_segments"
...
CREATE INDEX IF NOT EXISTS fno_v6_holdout_b_dataset_started_idx
...
CREATE INDEX IF NOT EXISTS fno_v6_holdout_b_history_symbol_idx
...
def _is_stale(run, *, now=None):
    ...
    if heartbeat is None:
        return True
    return (now or datetime.now(UTC)) - heartbeat > STALE_AFTER
    ...
    try:
        ...
        async def progress(update):
            try:
                ...
                # A transient heartbeat failure must not abort frozen computation.
                return
        ...
        with current_cash_symbol_aliases(provider):
            result = await build_holdout_b_dataset(
                provider,
                progress=progress,
                cache_get=cache_get,
                cache_put=cache_put,
        ...
        if result.get("status") != "COMPLETED":
            raise RuntimeError(
                "V6 Holdout B dataset build did not complete: "
                f"{result.get('status')} "
        ...
        try:
            await _update(
                settings.database_url,
                run_id,
                {"stage": "PERSISTING_FROZEN_DATASET"},
            )
        except Exception:
            pass
        try:
            ...
            # Frozen decisions remain unscored; explicit POST can resume safely.
            return
...
def register_fno_market_brain_v6_holdout_b_dataset_routes(
    app,
    settings,
    collector_auth,
):
    if getattr(
        app.state,
        "fno_market_brain_v6_holdout_b_dataset_registered",
        False,
    ):
        return
    app.state.fno_market_brain_v6_holdout_b_dataset_registered = True

    @app.post("/v1/internal/fno/v6-holdout-b-dataset/start")
    async def start(x_collector_token: str | None = Header(default=None)):
        ...
        if latest and latest.get("status") == "RUNNING":
            ...
            reason = (
                ...
                if same_commit
                else "INTERRUPTED_DEPLOYMENT_CHANGED"
            )
    ...
    @app.get("/v1/internal/fno/v6-holdout-b-dataset/status")
    async def status(x_collector_token: str | None = Header(default=None)):
        collector_auth(x_collector_token)
        await _ensure(settings.database_url)
        # Read-only polling can never relaunch the expensive build.
        return _summary(await _latest(settings.database_url))

    @app.get("/v1/internal/fno/v6-holdout-b-dataset/result")
