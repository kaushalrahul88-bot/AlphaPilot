...
def audit_directional_independence(families: list[dict]) -> dict:
    """Suppress duplicate/dependent evidence before a direction thesis is counted.

    A family must explicitly declare independence and a causal origin. At most one
    directional vote may be counted per causal origin. This prevents two wrappers
    around the same price move from being treated as separate confirmations.
    """
    counted: list[dict] = []
