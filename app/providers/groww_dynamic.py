...
class DynamicGrowwProvider(GrowwProvider):
    ...
    def _recommended_option(self,symbol,expiry,raw_chain,technical,min_rr=1.5):
        ...
        option_entry=premium if isinstance(premium,(int,float)) and premium>0 else None
        ...
        return {"underlying":symbol,"expiry":expiry,"direction":direction,"option_type":option_type,"strike":atm["strike"],"contract_label":f"{symbol} {expiry} {int(atm['strike'])} {option_type}","premium":premium,"option_entry":option_entry,"option_entry_basis":"CURRENT_OPTION_LTP","option_stop_loss":option_stop,"option_target1":option_target1,"option_target2":option_target2,"option_plan_ready":projection_ready,"projection_method":"Entry is anchored to the current option LTP. Execution levels use a premium-based intraday risk band: max premium loss is 30% below ₹10, 25% from ₹10-₹30, and 20% above ₹30. T1 uses the requested minimum R:R and T2 uses at least 2R. Delta/gamma projections are diagnostics only.","premium_risk_fraction":plan["risk_fraction"] if plan else None,"premium_risk_percent":plan["risk_percent"] if plan else None,"option_target1_percent":plan["target1_percent"] if plan else None,"option_target2_percent":plan["target2_percent"] if plan else None,"risk_model":"PREMIUM_PERCENT_INTRADAY","raw_greek_entry":raw_entry,"raw_greek_stop":raw_stop,"raw_greek_target1":raw_target1,"raw_greek_target2":raw_target2,"delta":delta,"gamma":gamma,"theta":greeks.get("theta"),"vega":greeks.get("vega"),"iv":atm.get(f"{prefix}_iv"),"open_interest":int(atm.get(f"{prefix}_oi") or 0),"volume":int(atm.get(f"{prefix}_volume") or 0),"underlying_ltp":spot,"underlying_entry":technical.get("entry"),"underlying_stop_loss":technical.get("stop_loss"),"underlying_target1":technical.get("target1"),"underlying_target2":technical.get("target2"),"selection_method":"ATM contract aligned with confirmed technical direction","warning":"Option execution entry uses the current option LTP and SL/targets are premium-based risk controls, not forecasts. Greek projections are diagnostic only; verify live price and spread before execution."}
