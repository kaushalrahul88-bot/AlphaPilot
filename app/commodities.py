...
async def commodity_mtf_scan(provider, symbol, min_rr=1.5):
    ...
    alpha=round(mean([_f(frames[tf].get("alpha_score"),50) for tf in frames]),1); fresh_all=all(bool(frames[tf].get("fresh")) for tf in frames)
    executable=session["is_open"] and fresh_all and action!="NO TRADE" and alpha>=65
    reference=next((frames[tf] for tf in ("15m","5m","1h") if frames[tf].get("signal")==action and frames[tf].get("status")=="SETUP"),None)
    return {"provider":"GROWW","mode":"MCX_COMMODITY_MTF","symbol":symbol,"contract":contract,"market_session":session,"timeframes":frames,"action":action,"alpha_score":alpha,"fresh_market_data":fresh_all if session["is_open"] else None,"execution_ready":bool(executable),"status":"READY" if executable else "SNAPSHOT" if not session["is_open"] else "WATCH","entry":reference.get("entry") if reference else None,"stop_loss":reference.get("stop_loss") if reference else None,"target1":reference.get("target1") if reference else None,"target2":reference.get("target2") if reference else None,"risk_reward":reference.get("risk_reward") if reference else None,"blockers":(["MCX market is closed; snapshot only."] if not session["is_open"] else [])+(["Fresh 5m/15m/1h commodity candles are required."] if session["is_open"] and not fresh_all else [])+(["At least 2 of 3 timeframes must agree on BUY or SELL."] if action=="NO TRADE" else [])+(["Average commodity Alpha must be at least 65."] if action!="NO TRADE" and alpha<65 else [])}
