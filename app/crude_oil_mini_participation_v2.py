...
def build_participation_observation(
    ...
    *,
    ...
    Direction is allowed only when fresh positioning evidence (OI) and auction
    acceptance agree. Price plus volume alone is explicitly dependent on local price
    and cannot masquerade as a second independent vote.
    """
    ...
    range_expanded = avg_range > 0 and recent_ranges[-1] > avg_range
    local_volume_expanded = avg_volume > 0 and recent_volumes[-1] > avg_volume
