...
from .copper_market_brain_direction_audit import run_market_brain_direction_audit_from_store
...
def _run_copper_current_mind_replay_job_sync(database_url:str):
    ...
    try:
        ...
        _copper_current_mind_replay_job={
            "status":"FAILED",
            "result":None,
            "error":str(exc)[:1000],
            "traceback":traceback.format_exc()[-4000:],
...
@app.get("/v1/internal/copper/current-mind-provider-discrepancies")
async def copper_current_mind_provider_discrepancies(x_collector_token:str|None=Header(default=None)):
    store=_collector_store(x_collector_token)
    try:return await run_provider_discrepancy_triangulation(get_provider(settings),store)
    except Exception as exc:_safe_upstream_error("Copper Current Mind provider discrepancy audit",exc)
...
async def copper_experience_memory(
    ...
    store=_collector_store(x_collector_token)
    try:
        return await run_experience_memory_from_store(
            store,max(1,min(int(sample_every_bars),12)),max(20,min(int(analogue_k),200)),
        )
    except Exception as exc:_safe_upstream_error("Copper Experience Memory",exc)

@app.get("/v1/internal/copper/event-path-backtest")
async def copper_event_path_backtest(
    sample_every_bars:int=3,
    x_collector_token:str|None=Header(default=None),
):
    store=_collector_store(x_collector_token)
    try:
        return await run_event_path_from_store(
            store,
            max(1,min(int(sample_every_bars),12)),
        )
    except Exception as exc:_safe_upstream_error("Copper event-path backtest",exc)

@app.get("/v1/internal/copper/market-brain-error-attribution")
async def copper_market_brain_error_attribution(
    sample_every_bars:int=3,
    x_collector_token:str|None=Header(default=None),
):
    store=_collector_store(x_collector_token)
    try:
        return await run_error_attribution_from_store(
            store,
            max(1,min(int(sample_every_bars),12)),
        )
    except Exception as exc:_safe_upstream_error("Copper Market Brain error attribution",exc)

@app.get("/v1/internal/copper/market-brain-direction-audit")
async def copper_market_brain_direction_audit(
    sample_every_bars:int=3,
    x_collector_token:str|None=Header(default=None),
):
    store=_collector_store(x_collector_token)
    try:
        return await run_market_brain_direction_audit_from_store(
            store,
            max(1,min(int(sample_every_bars),12)),
        )
    except Exception as exc:_safe_upstream_error("Copper Market Brain direction audit",exc)
