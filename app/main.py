...
from .commodity_click_replay import audit_identified_setups, run_frozen_extended_click_backtest, run_frozen_tuesday_phase_a, run_frozen_weekly_click_backtest, validate_frozen_tuesday_phase_a_data
...
settings=Settings(); app=FastAPI(title="AlphaPilot API",version="0.39.5"); parsed_origins=[x.strip() for x in settings.allowed_origins.split(",") if x.strip()]; parsed_origins=["*"] if "*" in parsed_origins else parsed_origins; app.add_middleware(CORSMiddleware,allow_origins=parsed_origins,allow_credentials=False,allow_methods=["*"],allow_headers=["*"]); TF=Literal["5m","15m","1h","1d"]
...
@app.get("/health")
async def health(): return {"ok":True,"service":"alphapilot-api","version":"0.39.5","provider":settings.market_data_provider.upper()}
