...
settings = Settings()
app = FastAPI(title="AlphaPilot API", version="0.2.0")
app.add_middleware(
    CORSMiddleware,
...
class ScanRequest(BaseModel):
    symbols: list[str] = Field(default_factory=lambda: ["NIFTY", "BANKNIFTY", "RELIANCE", "CRUDEOIL", "NATGAS", "GOLD"])
...
async def scan(request: ScanRequest):
    provider = get_provider(settings)
    return await provider.scan(request.symbols, request.timeframe, request.min_risk_reward)
