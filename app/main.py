...
settings=Settings(); app=FastAPI(title="AlphaPilot API",version="0.18.0"); parsed_origins=[x.strip() for x in settings.allowed_origins.split(",") if x.strip()]; parsed_origins=["*"] if "*" in parsed_origins else parsed_origins; app.add_middleware(CORSMiddleware,allow_origins=parsed_origins,allow_credentials=False,allow_methods=["*"],allow_headers=["*"]); TF=Literal["5m","15m","1h","1d"]
...
class FNOTrueBacktestRequest(BaseModel): symbols:list[str]=Field(default_factory=lambda:["RELIANCE"]); start_date:str; end_date:str; expiry:str; min_risk_reward:float=1.5; entry_before:str|None=None; max_trades:int=20
...
@app.get("/health")
async def health(): return {"ok":True,"service":"alphapilot-api","version":"0.18.0","provider":settings.market_data_provider.upper()}
