...
settings = Settings()
app = FastAPI(title="AlphaPilot API", version="0.22.0")
parsed_origins = [x.strip() for x in settings.allowed_origins.split(",") if x.strip()]
parsed_origins = ["*"] if "*" in parsed_origins else parsed_origins
...
def _safe_upstream_error(operation: str, exc: Exception):
    ...
    if upstream_path and "/v1/token/api/access" in upstream_path:
        ...
        source = "UPSTREAM"
    if status:
        detail = f"Groww {operation} failed: {source} HTTP {status}" + (f" at {upstream_path}" if upstream_path else "")
    ...
    logger.error("Groww %s error source=%s status=%s path=%s error=%r", operation, source, status, upstream_path, exc)
    raise HTTPException(status_code=429 if status == 429 else 502, detail=detail)
...
class StrategyResearchRequest(BaseModel):
    symbols: list[str] = Field(default_factory=lambda: ["RELIANCE", "SBIN", "AXISBANK"])
    start_date: str
    end_date: str
    target_r: float = 1.0


class StrategyPremiumReplayRequest(BaseModel):
    symbols: list[str] = Field(default_factory=lambda: ["RELIANCE", "SBIN", "AXISBANK"])
    start_date: str
    end_date: str
    strategy: Literal["VWAP_TREND", "ORB_30", "BREAKOUT_20"] = "VWAP_TREND"
    research_target_r: float = 1.0
    premium_min_risk_reward: float = 1.5
    max_trades: int = 30


class OptionNativeResearchRequest(BaseModel):
    symbols: list[str] = Field(default_factory=lambda: ["RELIANCE", "SBIN", "AXISBANK", "HDFCBANK", "ICICIBANK", "TATASTEEL", "HINDALCO", "ONGC", "INFY", "TCS"])
    start_date: str
    end_date: str
    research_target_r: float = 1.0
    premium_min_risk_reward: float = 1.5
    max_trades_per_strategy: int = 30
...
class FNOTrueBacktestRequest(BaseModel):
    symbols: list[str] = Field(default_factory=lambda: ["RELIANCE"])
    start_date: str
    end_date: str
    expiry: str | None = None
...
@app.get("/health")
async def health():
    return {"ok": True, "service": "alphapilot-api", "version": "0.22.0", "provider": settings.market_data_provider.upper()}
...
async def strategy_research(request: StrategyResearchRequest):
    ...
    try:
        return await run_strategy_research(get_provider(settings), symbols, request.start_date, request.end_date, request.target_r)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc))
    except Exception as exc:
        _safe_upstream_error("strategy research", exc)
...
async def strategy_premium_replay(request: StrategyPremiumReplayRequest):
    symbols = [s.upper() for s in request.symbols if s.strip()] or ["RELIANCE"]
    try:
        return await run_strategy_premium_replay(get_provider(settings), symbols, request.start_date, request.end_date, request.strategy, request.research_target_r, request.premium_min_risk_reward, request.max_trades)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc))
    except Exception as exc:
        _safe_upstream_error("strategy premium replay", exc)
...
async def option_native_research(request: OptionNativeResearchRequest):
    symbols = [s.upper() for s in request.symbols if s.strip()] or ["RELIANCE"]
    try:
        return await run_option_native_research(
            get_provider(settings),
            symbols,
            request.start_date,
            request.end_date,
            request.research_target_r,
            request.premium_min_risk_reward,
            request.max_trades_per_strategy,
            request.round_trip_cost_bps,
        )
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc))
    except Exception as exc:
        _safe_upstream_error("option-native research", exc)


@app.post("/v1/fno/history/probe")
async def fno_history_probe(request: FNOHistoryProbeRequest):
    try:
        return await probe_historical_option_candles(get_provider(settings), request.symbol.upper(), request.expiry, request.strike, request.option_type, request.interval, request.lookback_days)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc))
    except Exception as exc:
...
async def fno_history_replay(request: FNOPremiumReplayRequest):
    try:
        return await replay_option_trade(get_provider(settings), request.symbol.upper(), request.expiry, request.strike, request.option_type, request.trade_date, request.entry_time, request.min_risk_reward)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc))
    except Exception as exc:
...
async def fno_true_backtest(request: FNOTrueBacktestRequest):
    symbols = [s.upper() for s in request.symbols if s.strip()] or ["RELIANCE"]
    try:
        return await run_true_premium_backtest(get_provider(settings), symbols, request.start_date, request.end_date, request.expiry, request.min_risk_reward, request.entry_before, request.max_trades)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc))
    except Exception as exc:
        _safe_upstream_error("true premium F&O backtest", exc)
...
async def mcx_backtest(request: CommodityBacktestRequest):
    return await run_commodity_backtest(get_provider(settings), request.symbol, request.days, request.min_risk_reward, request.strength_threshold, request.slippage_bps, request.cost_bps)


@app.post("/v1/commodity/backtest/continuous")
async def mcx_continuous_backtest(request: CommodityBacktestRequest):
    return await run_continuous_commodity_backtest(get_provider(settings), request.symbol, request.days, request.min_risk_reward, request.strength_threshold, request.slippage_bps, request.cost_bps)


@app.get("/v1/news")
async def news(symbols: str, limit: int = 3):
    return await latest_market_news([x.strip().upper() for x in symbols.split(",") if x.strip()], max(1, min(int(limit), 5)))
...
async def mtf(request: MTFRequest):
    ...
    try:
        return await get_provider(settings).multi_timeframe_scan(symbols, request.timeframes, request.min_risk_reward)
    except Exception as exc:
        logger.error("MTF request crashed symbols=%s error=%s\n%s", symbols, repr(exc), traceback.format_exc())
        raise


@app.post("/v1/backtest")
async def backtest(request: BacktestRequest):
    return await run_backtest(get_provider(settings), [s.upper() for s in request.symbols if s.strip()] or ["RELIANCE"], request.start_date, request.end_date, request.min_risk_reward, request.entry_before)
...
async def fno_snapshot(request: SnapshotRequest):
    return await get_provider(settings).take_option_snapshot(request.symbol.upper(), request.expiry)
