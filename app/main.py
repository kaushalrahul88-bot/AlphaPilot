...
async def quote(symbol: str):
    return await get_provider(settings).quote(
        symbol.upper()
    )
...
async def candles(
    ...
    data = await get_provider(settings).candles(
        symbol.upper(),
        timeframe,
    )
    return {
...
async def multi_timeframe_scan(
    request: MultiTimeframeScanRequest,
):
    if len(request.timeframes) < 2:
        return {
            "error": (
                "Use at least two timeframes for "
