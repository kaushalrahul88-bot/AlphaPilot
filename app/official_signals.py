...
NOISE_TERMS = (
    ...
    "international -", "emissions factors", "page 1 of", "page 2 of",
)

GENERIC_TITLES = {
    "international", "database", "notifications", "speeches & media interactions", "draft directions",
    "daily treasury rates", "treasury securities upcoming auctions data",
}
...
async def official_signals(limit_per_group: int = 4) -> dict:
    limit = max(2, min(int(limit_per_group), 6)); groups: dict[str, list[dict]] = {}; all_rows: list[dict] = []; dropped_irrelevant = 0
    for group, cfg in OFFICIAL_WATCH.items():
        rows = await _fetch_google_news(str(cfg["query"]), f"official-signals:v1.4:{group}", min(14, limit * 4))
        ...
        for row in rows:
            ...
            seen.add(key)
            enriched.append({**row, "group": group, "source_type": _source_type(headline, source), "confidence": _confidence(source, headline), "novelty": _novelty(headline), "market_relevance": "MARKET_RELEVANT", "market_topics": _market_topics(headline), "impact": _impact(headline), "affected": _affected(group, headline), "primary_source_preferred": True})
    ...
    return {"mode": "ALPHAPILOT_OFFICIAL_SIGNALS_V1_4", "generated_at": datetime.now(timezone.utc).isoformat(), "research_only": True, "production_rules_changed": False, "groups": groups, "items": all_rows, "quality": {"accepted_items": len(all_rows), "dropped_irrelevant": dropped_irrelevant}, "method_note": "Official Signals v1.4 rejects generic index, database, jobs, event and archive pages before ranking market-relevant speeches/releases. Direct social-platform ingestion is only used when a supported authenticated API/feed exists."}
