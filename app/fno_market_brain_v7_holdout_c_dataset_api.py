"""Durable internal API for frozen V7 Historical Holdout C dataset build."""
...
from .fno_market_brain_v7_holdout_c_dataset import (
    PROTOCOL_ID,
    architecture_contract,
    build_holdout_c_dataset,
)
from .providers.factory import get_provider

UTC = timezone.utc
RUN_TABLE = "fno_market_brain_v7_holdout_c_dataset_runs"
CACHE_TABLE = "fno_market_brain_v7_holdout_c_history_segments"
...
CREATE INDEX IF NOT EXISTS fno_v7_holdout_c_dataset_started_idx
...
CREATE INDEX IF NOT EXISTS fno_v7_holdout_c_history_symbol_idx
...
async def _db_call(func, *args, retry_delays=None):
    ...
    for attempt in range(len(delays) + 1):
        try:
            ...
            last = exc
            if not _transient_db_error(exc) or attempt >= len(delays):
                raise
    ...
    with _connect(url) as conn:
        with conn.cursor() as cur:
            if increment:
                cur.execute(
                    f"""UPDATE {RUN_TABLE}
                    SET attempt_count=attempt_count+1,updated_at=NOW(),heartbeat_at=NOW(),progress_json=%s
...
def _encode_result(result):
    raw = json.dumps(result, ensure_ascii=True, separators=(",", ":"), default=str).encode("utf-8")
...
def _result_summary(result, blob_bytes=None):
    payload = {
        ...
        "missing_optional_context": result.get("missing_optional_context"),
        "frozen_5m_tape_hashes": result.get("frozen_5m_tape_hashes"),
        "safety": result.get("safety"),
        "result_blob_codec": RESULT_BLOB_CODEC,
    }
    if blob_bytes is not None:
        payload["result_blob_bytes"] = int(blob_bytes)
    return payload


def _complete_sync(url, run_id, result):
    blob = _encode_result(result)
    summary = _result_summary(result, len(blob))
    with _connect(url) as conn:
        with conn.cursor() as cur:
            cur.execute(
                f"""UPDATE {RUN_TABLE}
                SET status='COMPLETED',result_json=%s,result_blob=%s,progress_json=%s,
                error=NULL,traceback=NULL,updated_at=NOW(),heartbeat_at=NOW(),completed_at=NOW()
                WHERE run_id=%s AND status='RUNNING'""",
...
async def _complete(url, run_id, result):
    await _db_call(_complete_sync, url, run_id, result, retry_delays=FINAL_DB_RETRY_DELAYS_SECONDS)


def _full_result_sync(url, run_id):
    with _connect(url) as conn:
        with conn.cursor() as cur:
            cur.execute(f"SELECT result_blob,result_json FROM {RUN_TABLE} WHERE run_id=%s", (run_id,))
            row = cur.fetchone()
            if not row:
                return None
            blob, summary = row
            return _decode_result(blob) if blob is not None else summary
...
def _fail_sync(url, run_id, error, trace, result=None):
    detail = None
    if result:
        detail = {"status": result.get("status"), "protocol_id": result.get("protocol_id"), "safety": result.get("safety")}
    with _connect(url) as conn:
        with conn.cursor() as cur:
            cur.execute(
                f"""UPDATE {RUN_TABLE}
                SET status='FAILED',result_json=%s,error=%s,traceback=%s,progress_json=%s,
                updated_at=NOW(),heartbeat_at=NOW(),completed_at=NOW()
                WHERE run_id=%s AND status='RUNNING'""",
                (Jsonb(detail) if detail else None, error, trace, Jsonb({"stage": "FAILED"}), run_id),
            )
        conn.commit()
...
async def _fail(url, run_id, error, trace, result=None):
    await _db_call(
        _fail_sync, url, run_id, error, trace, result,
...
def _cache_get_sync(url, symbol, timeframe, window_start, window_end):
    with _connect(url) as conn:
        with conn.cursor() as cur:
            ...
            row = cur.fetchone()
            return list(row[0]) if row and isinstance(row[0], list) else None


async def _cache_get(url, symbol, timeframe, window_start, window_end):
    return await _db_call(_cache_get_sync, url, symbol, timeframe, window_start, window_end)
    ...
    with _connect(url) as conn:
        with conn.cursor() as cur:
            cur.execute(
                ...
                DO UPDATE SET rows_json=EXCLUDED.rows_json,row_count=EXCLUDED.row_count,updated_at=NOW()""",
                (PROTOCOL_ID, symbol, timeframe, window_start, window_end, Jsonb(list(rows)), len(rows)),
            )
        conn.commit()


async def _cache_put(url, symbol, timeframe, window_start, window_end, rows):
    await _db_call(_cache_put_sync, url, symbol, timeframe, window_start, window_end, rows)
...
def _is_stale(run, now=None):
    ...
    return heartbeat is None or (now or datetime.now(UTC)) - heartbeat > STALE_AFTER
...
def _summary(run):
    ...
    if run.get("status") == "COMPLETED":
        for key in (
            "experiment", "decision_counts", "data_coverage", "context_coverage",
            "missing_optional_context", "frozen_5m_tape_hashes", "result_blob_bytes",
        ):
...
async def _run(settings, run_id):
    ...
    try:
        ...
        async def cache_get(symbol, timeframe, window_start, window_end):
            return await _cache_get(settings.database_url, symbol, timeframe, window_start, window_end)

        async def cache_put(symbol, timeframe, window_start, window_end, rows):
            await _cache_put(settings.database_url, symbol, timeframe, window_start, window_end, rows)

        result = await build_holdout_c_dataset(
            provider,
            progress=progress,
            cache_get=cache_get,
            cache_put=cache_put,
        ...
        if result.get("status") != "COMPLETED":
            raise RuntimeError(
                "V7 Holdout C dataset build did not complete: "
                f"{result.get('status')} "
        ...
        try:
            ...
            # Explicit POST can resume safely from transport cache if final DB persistence fails.
            return
...
def register_fno_market_brain_v7_holdout_c_dataset_routes(app, settings, collector_auth):
    if getattr(app.state, "fno_market_brain_v7_holdout_c_dataset_registered", False):
        return
    app.state.fno_market_brain_v7_holdout_c_dataset_registered = True

    @app.post("/v1/internal/fno/v7-holdout-c-dataset/start")
    async def start(x_collector_token: str | None = Header(default=None)):
        ...
        if latest and latest.get("status") == "RUNNING":
            ...
            await _interrupt(
                settings.database_url,
                latest["run_id"],
                "INTERRUPTED_STALE_RUN_REPLACED" if same_commit else "INTERRUPTED_DEPLOYMENT_CHANGED",
            )
    ...
    async def status(x_collector_token: str | None = Header(default=None)):
        collector_auth(x_collector_token)
        await _ensure(settings.database_url)
        # Polling is read-only and never starts/restarts the expensive builder.
        return _summary(await _latest(settings.database_url))

    @app.get("/v1/internal/fno/v7-holdout-c-dataset/result")
