...
MODE = "CRUDE_OIL_MINI_DIRECTION_BRAIN_V2_SHADOW_LEGACY_SNAPSHOT"
...
def _thesis_from_families(families: list[dict]) -> dict:
    ...
    if bullish and bearish:
        return {"direction": "UNKNOWN", "confidence": "CONFLICTED", "state": "INDEPENDENT_FAMILY_CONTRADICTION", "supporting_families": [], "opposing_families": sorted(bullish + bearish)}
    supporting = bullish or bearish
    if len(supporting) < 2:
        return {"direction": "UNKNOWN", "confidence": "WEAK", "state": "INSUFFICIENT_INDEPENDENT_CONFIRMATION", "supporting_families": sorted(supporting), "opposing_families": []}
    return {"direction": "BULLISH" if bullish else "BEARISH", "confidence": "STRONG" if len(supporting) >= 3 else "MODERATE", "state": "COHERENT_DIRECTION_THESIS", "supporting_families": sorted(supporting), "opposing_families": []}


def evaluate_direction_brain_v2_legacy_snapshot(*, click_timestamp: str, snapshot: dict, profile: dict, context_records: list[dict], direction_memory_cases: list[dict] | None = None) -> dict:
