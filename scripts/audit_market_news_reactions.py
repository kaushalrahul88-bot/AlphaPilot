...
def audit(news_payload:dict,candles:list[dict],*,as_of:str)->dict:
    ...
    for raw in news_payload.get("records") or []:
        event=_event(raw)
        event_raw=event.get("available_at") or event.get("published_at")
        ...
        if event_ts is not None and coverage_end is not None and event_ts > coverage_end:
            ...
            rows.append({"event":event,"status":"OUTSIDE_CANDLE_COVERAGE",
                         "coverage_status":"OUTSIDE_CANDLE_COVERAGE","market_coverage":coverage})
            continue
        ...
        if window.get("status")!="READY":
            coverage_status="INSUFFICIENT_REACTION_WINDOW"
            coverage_counts[coverage_status]+=1
            rows.append({"event":event,"status":window.get("status"),"coverage_status":coverage_status,
    ...
    return {"mode":"MARKET_NEWS_REACTION_AUDIT_V1","outcome_blind":True,"outcomes_read":False,
            "as_of":as_of,"events":len(news_payload.get("records") or []),"market_coverage":coverage,
            "coverage_counts":dict(sorted(coverage_counts.items())),
            "classified":sum(reaction_counts.values()),"reaction_counts":dict(sorted(reaction_counts.items())),
            ...
            "guardrails":["Trade outcomes and P&L are not accepted as audit inputs.",
                          ...
                          "Events later than the frozen candle coverage are OUTSIDE_CANDLE_COVERAGE and cannot be classified.",
